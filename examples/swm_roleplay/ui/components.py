...
def avatar_card(role_id: str, name: str, title: str, state: str = "idle",
    ...
    if satisfaction is not None:
        ...
        # Smooth red → amber → green gradient: the fill colour reflects the
        # value, not just its width. 0% shows red, 100% shows green.
        meter = (
            f'<div class="wm-meter cx-meter-shimmer">'
            f'<span style="width:{pct}%;background:linear-gradient(90deg,'
            f' hsl(0,68%,46%) 0%, hsl(40,70%,46%) 50%,'
            f' hsl(125,55%,38%) 100%);"></span></div>'
        )
        # agree / partial / disagree / absent — derived from BOTH the value
        # and the state so a forced "block" reads as disagrees even if the
        # rolling satisfaction estimate hasn't caught up yet.
        if state == "block" or pct < 30:
            sat_label = '<span class="wm-sat-label disagrees">disagrees</span>'
        elif state == "walkout":
            sat_label = '<span class="wm-sat-label absent">disengaged</span>'
        elif state in ("endorse",) or pct >= 78:
            sat_label = '<span class="wm-sat-label agrees">agrees</span>'
        elif state == "wary" or pct < 65:
            sat_label = '<span class="wm-sat-label partial">partially agrees</span>'
        else:
            sat_label = '<span class="wm-sat-label agrees">agrees</span>'
...
def background_fx() -> str:
    """Fixed, behind-content atmospheric layer: grain + drifting particles +
    sparkles. The bottom conveyor band was removed — it read as a permanent
    horizontal bar at the bottom of the viewport while scrolling.
    """
    particles = ""
    for i in range(12):
        left = 4 + (i * 8) % 92
