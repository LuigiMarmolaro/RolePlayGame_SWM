...
def _message_content_to_text(content):
    """
    Normalize provider-specific chat message content into text.

    Some providers return LangChain message content as a list of parts instead
    of a plain string. Agent post-processing expects text.
    """
...
class Agent:
    ...
    def _get_llm_response(self, messages, update_tool_memory: bool = False) -> Tuple[AIMessage, List[Event]]:
        ...
        response.content = _message_content_to_text(response.content)
